...
if(isset($_POST['submit'])){
	...
	$remaining_stock=$remaining_stock - $sale_quantity;
	...
	$query="INSERT INTO sales(product_id,price,quantity,date,total_sales,selling_price,cost,expected_sales) ";
	$query.="VALUES('{$the_product_id}','{$product_price}','{$sale_quantity}','{$sales_date}','{$total_sales_price}','{$sold_for}','{$product_cost}','{$expected_sales}' )";
	$insert=mysqli_query($connection,$query);
	if(!$insert){
		die("query failed".mysqli_error($connection));
	}else{

	echo "<p class='alert alert-success' style='margin-top:30px'>successfully SUbmitted<p>".""."<a href='total_sales.php'>View all sales</a>";
}
